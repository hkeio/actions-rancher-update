...
async function main() {
  ...
  if (!stack || !stack.data[0]) {
    throw new Error(
      "Could not find stack name. Check the stack_name input. Deploy failed!"
    );
  }
  const stackId = stack.data[0].id;

  // Check the service
  const service = await rancherApi.get(
    `/services?name=${SERVICE_NAME}&stackId=${stackId}`
  );
  if (!service || !service.data[0]) {
    throw new Error(
      "Could not find service name. Check the service_name input. Deploy failed!"
    );
  }
